...
const updateAdmin = async(req,res)=>{
    ...
    if(req.files){
        const  filePath = path.join('D:','ws-cube','react','Next_Js','backend','src','uploads','admin');

        if(req.files.logo){
            data.logo = req.files.logo[0].filename

            if(fs.existsSync(`${filePath}/${predata.logo}`)){
                fs.unlinkSync(`${filePath}/${predata.logo}`)
            }
        }

        if(req.files.favicon){
            data.favicon = req.files.favicon[0].filename
            if(fs.existsSync(`${filePath}/${predata.favicon}`)){
                fs.unlinkSync(`${filePath}/${predata.favicon}`)
            }
        }

        if(req.files.footer_icon){
            data.footer_icon = req.files.footer_icon[0].filename
            if(fs.existsSync(`${filePath}/${predata.footer_icon}`)){
                fs.unlinkSync(`${filePath}/${predata.footer_icon}`)
            }
        }
